// Initial Texts
export const TEXT_INITIAL_1 = `
  Let's get familiar with the rules of the Prisoner's Dilemma. Each round, you and your opponent choose to COOPERATE or DEFECT. 
  The diagram will update with point values based on what you and your opponent decide to do.
`;
export const TEXT_INITIAL_2 = `
  Let's say the other player defects. What will you do?
`;

// Round 1 Outcomes
export const TEXT_COOPERATE_1 = `
  If you cooperate & they defect, you get nothing while they gain five points 
  (score: +5 vs +0). However, if you both defect, you both get a small amount of points
  (score: +1 vs +1).
`;
export const TEXT_COOPERATE_2 = `
  Let's say the other player cooperates. 
  What will you do now?
`;

export const TEXT_DEFECT_1 = `
  If you cooperate & they defect, you gain nothing while they gain five points. 
  (score: +5 vs +0). However, if you both defect, you both get a small amount of points
  (score: +1 vs +1).
`;
export const TEXT_DEFECT_2 = `
  Let's say the other player cooperates. 
  What will you do now?
`;

// Round 2 Outcomes
export const TEXT_COOPERATE_AGAIN_1 = `
  If you both cooperate, you both give up a potential two points to gain three. 
  (score: +3 vs +3) However, if you defect & they cooperate, you gain five points while they gain nothing. 
  (score: +5 vs +0).
`;
export const TEXT_COOPERATE_AGAIN_2 = `
  And that's our dilemma.`;

export const TEXT_DEFECT_AGAIN_1 = `
  If you both cooperate, you both give up a potential two points to gain three. 
  (score: +3 vs +3) But if you defect & they cooperate, you gain five points while they gain nothing.
  (score: +0 vs +5).
`;

export const TEXT_DEFECT_AGAIN_2 = `
  And that's our dilemma.
`;